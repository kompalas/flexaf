...
def prune_l1_norm_mlp(model, sparsity_ratio):
...
def prune_l2_norm_mlp(model, sparsity_ratio):
    for i, W in enumerate(model.coefs_):
...
def prune_quantize_mlp(classifier, sparsity_levels, input_precision, weight_precisions, test_data, hw_eval_dir, prefix=''):
    ...
    for sparsity in sparsity_levels:
        ...
        pruned_model = prune_l2_norm_mlp(classifier._clf, sparsity)
...
def prune_quantize_keras(classifier, sparsity_levels, input_precision, weight_precisions, test_data, hw_eval_dir):
    """Prune and quantize a Keras classifier."""
    raise NotImplementedError("Keras pruning and quantization not implemented yet.")
    results = []
    for sparsity in sparsity_levels:
        # pruned_model = prune_l1_norm(model, sparsity)
        pruned_model = prune_l2_norm_mlp(classifier._clf, sparsity)
        ...
        _results = quantize_classifier(classifier, input_precision, weight_precisions, test_data, hw_eval_dir, prefix=f'{int(100 * sparsity)}l2norm')
        _results = [r | {'sparsity': sparsity} for r in _results]
        results.extend(_results)
    return results
...
def run_statistical_feature_selection(args):
    ...
    train_data, test_data, categ_labels, feature_costs, filtered_params, _ = prepare_feature_data(args)
    ...
    for num_features in feature_sizes:
        ...
        for fs_name, selector in feature_selectors.items():
            ...
            for clf_name in classifiers:
                ...
                elif clf_type == ClassifierType.FCNN:
                    results = prune_quantize_keras(clf, sparsity_levels, input_precision, weight_precisions, (x_test_sub, y_test), hw_eval_dir)
                else:
